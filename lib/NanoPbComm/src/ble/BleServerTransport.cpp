...
void BleServerTransport::onConnect(NimBLEServer *server, NimBLEConnInfo &connInfo) {
    _connected = true;
    // 1.x needed a second onConnect overload to see the connection handle at
    // all; 2.x passes it to the single callback. Kept because the ping watchdog
    // calls disconnect() with it when the controller stops answering.
