...
HUNKS = [
    ...
    (
        ...
        "                }\n"
        "                // Zero means either the register has not moved on yet, which happens\n"
        "                // when this handler beats it, or the DMA lapped the whole pool. The\n"
        "                // two are indistinguishable and it does not matter: a full lap has\n"
        "                // already drained every buffer, so there is nothing a refill could\n"
        "                // have saved. VSYNC squares that case up against the beam.\n"
        "                if (advanced > fills) {\n"
        "                    fills = advanced;\n"
        "                }\n"
        "            }\n"
        ...
        "            rgb_panel->bb_eof_count++;\n"
        "            portEXIT_CRITICAL_ISR(&rgb_panel->spinlock);\n"
        "            if (lcd_rgb_panel_fill_bounce_buffer(rgb_panel, rgb_panel->bounce_buffer[bb])) {\n"
        "                need_yield = true;\n"
        "            }\n"
        "        }\n"
        "        {\n"
    ...
    (
        ...
        "        } else {\n"
        "            // Counting more buffers than a frame holds would walk the fill position\n"
        "            // ahead of the beam, which scrolls the picture rather than shifting it.\n"
        "            // Nothing in the handler above can produce it; count it and say so.\n"
        "            gm_rgb_over_count++;\n"
        "        }\n"
