...
DRAM_ATTR static const lcd_init_cmd_t st7701_2_8_inches[] = {
    ...
    {0xC0, {0x3B, 0X00}, 0x02},
    {0xC1, {0x10, 0x0C}, 0x02},
    // INVSET first byte 0x31 (was 0x07): the shipped value drove the glass in
    // its worst-flicker inversion mode — smooth gradients shimmered constantly
    // at any refresh rate. 0x31 (dot-inversion family, as used by LilyGo
    // T-Panel on the same controller) eliminates it entirely.
    {0xC2, {0x31, 0x0A}, 0x02},
    {0xC7, {0x00}, 0x01},
    {0xCC, {0x10}, 0x01},
    {0xCD, {0x08}, 0x01}, // 用565时屏蔽    666打开
    ...
    {0xb3, {0x80}, 0x01},
    {0xb5, {0x4E}, 0x01},
    {0xb7, {0x8C}, 0x01}, // source-amp bias at IC default (LilyGo shipped reduced 0x85)
    {0xb8, {0x20}, 0x01},
    {0xc1, {0x78}, 0x01},
    {0xc2, {0x78}, 0x01},
    {0xd0, {0x88}, 0x01},
