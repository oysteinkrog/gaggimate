...
constexpr float STOP_POS[6] = {0.00f, 0.12f, 0.30f, 0.55f, 0.80f, 1.00f};
constexpr uint32_t EMBER_HEX[6] = {0xf4a94a, 0xe2751f, 0xb8420f, 0x6b1a08, 0x2b0a06, 0x0a0604};
constexpr uint32_t GOLD_HEX[6] = {0xffe0a3, 0xffb454, 0xe07a1a, 0x8a2e0a, 0x331006, 0x0b0704};
...
uint16_t *palette = nullptr;  // 256 entries, rebuilt on warmth change
uint8_t *radiusLUT = nullptr; // r^2>>RSHIFT -> normalized radius byte
const uint8_t *noise = nullptr;
uint8_t lastWarmth = 255, lastGlow = 255;
...
void frame(uint32_t tMs, int, int, const uint8_t p[4]) {
    if (p[0] != lastWarmth) {
        buildRamp565(palette, STOP_POS, EMBER_HEX, GOLD_HEX, 6, (p[0] * 256) / 100);
        lastWarmth = p[0];
    }
    if (p[1] != lastGlow) {
        buildRadiusLut(p[1]);
    ...
    const float s1 = sin1024((tMs * STEP1) >> 22) * (1.0f / SIN_AMP);
    const float s2 = sin1024(((tMs * STEP2) + PHOFF2) >> 22) * (1.0f / SIN_AMP);
    const float s3 = sin1024(((tMs * STEP3) + PHOFF3) >> 22) * (1.0f / SIN_AMP);
    g_breathe = static_cast<int>(pulseGain * (0.30f * s1 + 0.15f * s2 + 0.05f * s3) * 70.0f);
    g_flickerAmp = (10 * p[2]) / 100;
    g_sx = static_cast<int>((tMs * 6u) >> 10) & 255;
    g_sy = static_cast<int>((tMs * 4u) >> 10) & 255;
}
...
const BgAnimation bg_anim_ember = {
    "ember",
    "Ember",
    {{"warmth", "Warmth", 55}, {"glow", "Glow size", 45}, {"flicker", "Flicker", 20}, {"pulse", "Pulse", 50}},
    init,
    frame,
    band,
};
